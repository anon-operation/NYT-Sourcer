...
st.title("The Sourcer...lite version")
...
two=st.sidebar.subheader("This app shows names that appear in New York Times stories during September 2020.")
...
if moreInfo:
    ...
    st.sidebar.subheader("This is a sample of a larger database that includes stories going back several years.")
    st.sidebar.subheader("The data allows users to see whom was quoted over time and to hold media accountable.")
    st.sidebar.subheader("The source code is availible for inspection at: https://github.com/anon-operation/Sourcer-app.git")

##
#################  Main Graph
##
modifyChart=st.checkbox("Click here for more options")
if modifyChart:
    section_selectbox=st.selectbox("What section would you like to see?",
                                   ("All sections", "Health", "U.S.", "Business", "Upshot",
                                    "Magazine", "Briefing", "Real Estate",
                                    "Opinion", "N.Y. Region", "World",
                                    "Climate", "T Magazine", "Technology",
                                    "Style", "Sports", "Arts"))
    attribute_selectbox=st.selectbox("Type of attribute", ("All","NLTK", "NER", "Quotations"))
    ...
    elif attribute_selectbox=="Quotations":
        st.text(attribute_selectbox+" means that all names identified by the Natural Language processor will be displyed.")
        attributeChoice='quotes_method1'
    ...
    if startDate <= endDate:
        if section_selectbox=="All sections":
            ...
            if lowercaseSelection=="u.s.":
                lowercaseSelection="u.s."
            if lowercaseSelection=="business":
                lowercaseSelection="business day"
            if lowercaseSelection=="upshot":
                lowercaseSelection="the upshot"
            elif lowercaseSelection=="n.y. region":
                lowercaseSelection="new york"
    ...
    if breakDownSwitch1:
        st.text("Chart shows names that appear more than 100 times in September.")
        st.text("If the chart looks empty, it's because it's trying to display too much data.")
        st.text("Hover your mouse over the chart and click the epand icon in the top right or narrow the search parameters.")
        query1="SELECT * FROM ARTICLES WHERE Type LIKE 'quotes_method1';"
        df= load_data(query1)
        df['Tally']=df.groupby('Name')['Name'].transform('count')
        df=df[df['Tally'] > 100]
        fig=px.bar(df, x="Name", y="Count", color="Section",
        ...
        ##
        query="SELECT Name, count(*) FROM ARTICLES WHERE Type LIKE 'quotes_method1' GROUP BY Name;"
...
def timeSeriesQuery(nameInput,attribute,sectionChoice):
    searchName=nameInput.lower()
    query="SELECT Name,Date,Title,Type,Section, Count FROM ARTICLES WHERE Type LIKE "+"'"+attribute+"' "+ " AND Section LIKE "+"'"+sectionChoice+"'"+   ";"
...
def TsChart2(dataFrame, name_input):
    labString="Number of articles the names appears in"
    labDict={"Week":"Week / Year", "Count":labString}
...
##########
modifyChart2=st.checkbox("Click for more options")
if modifyChart2:
    section_selectbox2=st.selectbox("What section do you want to see?",
                                   ("All sections", "Health", "U.S.", "Business", "Upshot",
                                    ...
                                    "Climate", "T Magazine", "Technology",
                                    "Style", "Sports", "Arts"))
    lowercaseSelection2=section_selectbox2.lower()
    if lowercaseSelection2=="u.s.":
        lowercaseSelection2="u.s."
    if lowercaseSelection2=="all sections":
        lowercaseSelection2='%'
    if lowercaseSelection2=="upshot":
        lowercaseSelection2="the upshot"
    if lowercaseSelection2=="business":
        lowercaseSelection2="business day"
    elif lowercaseSelection2=="n.y. region":
        lowercaseSelection2="new york"
    attribute_selectbox2=st.selectbox("Attribute", ("All","NLTK", "NER", "Quotations"))
...
##        st.text(attribute_selectbox+" means that all names found near the keywords 'recalls', 'recalled', 'thinks', 'think', 'describe', 'agreed', 'agrees', 'describes', 'described', 'points', 'pointed', 'point', 'indicates', 'indicate', and 'indicated' will be displyed.")
    ...
    elif attribute_selectbox2=="Quotations":
##        st.text(attribute_selectbox+" means that all names identified by the Natural Language processor will be displyed.")
        attributeChoice2='quotes_method1'
else:
    attributeChoice2='quotes_method1'
    lowercaseSelection2='%'
...
##########
if addName:
    res=timeSeriesQuery(name_user_input,attributeChoice2,lowercaseSelection2)
    ...
    res2=timeSeriesQuery(name_user_input2,attributeChoice2,lowercaseSelection2)
    resName2=res2[0]
    ...
    try:
        ...
        st.error("There are no results. Try expanding your search.")
    if (res2[0].empty) and (res[0].empty):
        st.error("There are no results. Try expanding your search.")
    else:
...
@st.cache(max_entries=10)
def algCompChart(dataFrame, name_input,userInput):
    datas=dataFrame.groupby(['Type'])['Count'].sum()
    labString="Names recognized in article"
    labDict={"Type":"Recognition Algorithm", datas[1]:"Names Identified"}
    titleString="Names recognized in articles with '"+userInput+"' in the the title"
    namefig=px.bar(datas,
                   labels=labDict,
                   title=titleString)
##    namefig=px.bar(datas, x='Type', y='Count',
...
##    optionBox=st.checkbox("Show each name")
    ...
    if article_user_input=="":
        ...
        algChart=algCompChart(art,article_user_input,article_user_input)
        ...
        st.subheader("The 'quotes' method is the best for finding people that were actually quoted in the story.")    
##
##    else:
##        if optionBox:
