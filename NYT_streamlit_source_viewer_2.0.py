...
if modifyChart:
    try:
        ...
        if startDate <= endDate:
            timeQueryString = " BETWEEN "+"'"+qDate1+"'"+" AND "+"'"+qDate2+"'"+" ;"
            data=runPandaQuery ("*","Date",timeQueryString)  ##(fieldName,queryFieldName,queryValue)  ("*","Section"," LIKE 'healt%';")
            df=pd.DataFrame(data)
            df['Date']=pd.to_datetime(df['Date'], format='%Y-%m-%dT%H:%M:%S')
            df['Publication Date']=df['Date']
            if section_selectbox=="All sections":
                ...
                try:
                    ...
                    if lowercaseSelection=="u.s.":
                        df3=df[df['Section'].str.contains("us")]
                        df3=df3[~df3['Section'].str.contains("ness")]
                        fig=px.bar(df3, x="Name", y="Count", color="Section",
                            hover_data=["Title", "Url", "Date"])
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        df3=df[df['Section'].str.contains(lowercaseSelection)]
                        fig=px.bar(df3, x="Name", y="Count", color="Section",
                                hover_data=["Title", "Url", "Date"])
                        st.plotly_chart(fig, use_container_width=True)
                except:
